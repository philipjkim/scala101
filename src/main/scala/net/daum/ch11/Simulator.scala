...
object Simulator {
  ...
  abstract class BasicCircuitSimulation extends Simulation {
    ...
    def probe(name: String, wire: Wire) {
      wire addAction { () =>
        println("%s %d new_value=%b".format(name, currentTime, wire.getSignal))
      }
    }
  }
  ...
  abstract class Simulation {
    ...
    def run() {
      afterDelay(0) {
        println("*** simulation started, time = %d ***" format currentTime)
      }
...
object MySimulation extends Simulator.CircuitSimulation {
  def InverterDelay = 1
  def AndGateDelay = 3
  def OrGateDelay = 5
